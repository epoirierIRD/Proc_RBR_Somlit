...
# ################################################################
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# this is a work main that has been used to create a function wuithout success using trim tool 
# to split a big rsk in several rsk files by day
# chatgpt won
...
with pyrsk.RSK(mrsk_file) as rsk:
    ...
    # read the data first
    # rsk.readdata(np.datetime64('2024-07-23'), np.datetime64('2024-08-03'))
    # rsk.trim(reference = 'time', range=[rsk.regions[2].tstamp1,rsk.regions[2].tstamp2], action = 'remove')
    # rsk.trim(reference = 'time', range=[np.datetime64('2024-07-19'),np.datetime64('2024-07-23')], action = 'remove')
    # liste = rsk.trim(reference = 'time', range=[np.datetime64('0'), np.datetime64('2024-06-30')], action = "NaN")
    # liste = rsk.trim(reference= "index", range=[0, 1000], profiles=1)
    # liste = rsk.trim(reference = 'time', range=[np.datetime64('2024-07-26'), np.datetime64('2024-08-30')])
    # rsk.RSK2RSK(outputDir = os.path.dirname(mrsk_file), suffix = 'hello') 
    
    rsk.readdata()
    # profiles = rsk.getprofilesindices(range(0, 1), direction="both")
    
    # for profileIndices in profiles:
    #     print(rsk.data[profileIndices])
    
    # this function below works to write one csv per profile
        
    rsk.RSK2CSV(profiles = range(0,15), outputDir='/home/epoirier/Documents/PROJETS/2025/Proc_RBR_Somlit/summer_2024/temp/')
...
'''   
    for i in df.index:
        print(np.datetime64(df.iloc[i+1]['start_dt']))
...
# df,rsk = split_rsk_file(mrsk_file)
